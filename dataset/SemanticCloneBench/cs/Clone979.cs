...
*  Stack overflow Question #:29047477
*  Stack Overflow answer #:29047696
*  And Stack Overflow answer#:29047696
*/
...
private static Dictionary < string, int > BuildIndex (List < TagType < object > > tagList) {
    ...
    for (int i = 0; i < tagList.Count; i ++) {
        var tag = tagList [i];
        if (! index.ContainsKey (tag.FieldTag))
            index.Add (tag.FieldTag, i);
    }
    return index;
}
